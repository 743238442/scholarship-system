...
| review_no | VARCHAR(50) | UNIQUE | | 评审编号 |
| academic_year | VARCHAR(20) | NOT NULL | | 学年 |
| application_status | VARCHAR(20) | | 'pending' | 申请状态 |
| review_status | VARCHAR(20) | | 'not_reviewed' | 评审状态 |
| academic_score | DECIMAL(5,2) | | | 学业成绩分数 |
| conduct_score | DECIMAL(5,2) | | | 行为表现分数 |
| extra_score | DECIMAL(5,2) | | | 加分项分数 |
| total_score | DECIMAL(5,2) | | | 总分数 |
| review_opinion | TEXT | | | 评审意见 |
| reviewer_notes | TEXT | | | 评审备注 |
...
| type | VARCHAR(20) | | 'scholarship' | 公示类型 |
...
| end_date | DATE | | | 结束日期 |
| publish_date | DATE | | CURRENT_DATE | 发布日期 |
...
CREATE INDEX idx_announcement_created_by ON tbl_announcement(created_by);
```
