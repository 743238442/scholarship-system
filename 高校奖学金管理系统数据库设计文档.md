...
CREATE INDEX idx_announcement_deleted ON announcement(deleted);
CREATE INDEX idx_announcement_published_by ON announcement(published_by);
```
...
- 在实际系统中，AnnouncementRepository提供了完整的软删除支持
...
### 4.3 公告信息视图 (v_announcement_active)
**视图功能**：提供当前有效的公告信息
...
-- 限制敏感表访问
REVOKE ALL ON user FROM scholarship_app;
GRANT SELECT (id, username, email, role, is_active) ON user TO scholarship_app;
```
...
```sql
-- 学业记录表按学年分区
CREATE TABLE academic_record_partitioned (
    LIKE academic_record INCLUDING ALL
) PARTITION BY RANGE (academic_year);

-- 创建年度分区
CREATE TABLE academic_record_2023 PARTITION OF academic_record_partitioned
    FOR VALUES FROM ('2023') TO ('2024');

CREATE TABLE academic_record_2024 PARTITION OF academic_record_partitioned
