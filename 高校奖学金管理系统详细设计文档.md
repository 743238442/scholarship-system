...
CREATE INDEX idx_announcements_top ON announcements(is_top);
...
##### 3.2.5.1 软删除功能实现
公示表采用软删除机制，删除操作不会真正删除数据，而是通过设置 `deleted=true` 和 `deleted_at` 时间戳来标记数据已删除。所有查询操作会自动过滤掉 `deleted=true` 的记录。
...
public interface AnnouncementRepository extends JpaRepository<Announcement, Long> {
    ...
    // 查询所有未删除且激活的公告
    List<Announcement> findByIsActiveTrueAndDeletedIsFalse();
