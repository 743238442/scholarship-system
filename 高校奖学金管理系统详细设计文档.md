...
├─────────────────────────────────────────────────────────┤
│                     数据存储层                           │
│            PostgreSQL 13+ + Simple Cache (内存缓存)      │
└─────────────────────────────────────────────────────────┘
```
...
##### 3.2.5.1 软删除功能实现
公告表采用软删除机制，删除操作不会真正删除数据，而是通过设置 `deleted=true` 和 `deleted_at` 时间戳来标记数据已删除。所有查询操作会自动过滤掉 `deleted=true` 的记录。

**软删除实现逻辑：**
- 删除操作：设置 `deleted=true` 和 `deleted_at=CURRENT_TIMESTAMP`
- 查询操作：添加 `WHERE deleted=false` 条件
- 恢复操作（可选）：设置 `deleted=false` 和 `deleted_at=null`
...
public interface AnnouncementRepository extends JpaRepository<Announcement, Long> {
    // 查询所有未删除的公告
    List<Announcement> findByDeletedIsFalse();
    
    // 根据ID查询未删除的公告
    Optional<Announcement> findByIdAndDeletedIsFalse(Long id);
    
    // 查询所有未删除的公告并按发布时间倒序排序
    List<Announcement> findByDeletedIsFalseOrderByPublishedAtDesc();
    
    // 查询所有未删除且置顶的公告
    List<Announcement> findByIsTopTrueAndDeletedIsFalse();
}
```
...
public class AnnouncementService {
    ...
    public List<Announcement> getAllAnnouncements() {
        return announcementRepository.findByDeletedIsFalse();
    }
...
public class AnnouncementController {
    ...
    public ResponseEntity<Announcement> createAnnouncement(@RequestBody Announcement announcement) {
        Announcement createdAnnouncement = announcementService.publishAnnouncement(announcement);
        return ResponseEntity.created(URI.create("/api/announcements/" + createdAnnouncement.getId()))
    ...
    public ResponseEntity<Announcement> updateAnnouncement(@PathVariable Long id, @RequestBody Announcement announcementDetails) {
        ...
        if (announcement.isPresent()) {
            ...
            updatedAnnouncement.setTitle(announcementDetails.getTitle());
            updatedAnnouncement.setContent(announcementDetails.getContent());
            updatedAnnouncement.setPriority(announcementDetails.getPriority());
            updatedAnnouncement.setIsTop(announcementDetails.getIsTop());
            updatedAnnouncement.setIsPublic(announcementDetails.getIsPublic());
            updatedAnnouncement.setIsActive(announcementDetails.getIsActive());
            updatedAnnouncement.setStartDate(announcementDetails.getStartDate());
            updatedAnnouncement.setEndDate(announcementDetails.getEndDate());
            updatedAnnouncement.setPublishedBy(announcementDetails.getPublishedBy());
            
            Announcement savedAnnouncement = announcementService.updateAnnouncement(updatedAnnouncement);
...
public class AdminAnnouncementController {
    ...
    // 保存公告
    @PostMapping
    public String saveAnnouncement(@ModelAttribute Announcement announcement, Authentication authentication) {
        // 获取当前登录用户ID
        Long currentUserId = ((User) authentication.getPrincipal()).getId();
        announcement.setPublishedBy(currentUserId);
...
LEFT JOIN review r ON s.id = r.student_id
...
LEFT JOIN review r ON st.id = r.scholarship_type_id
...
public interface AnnouncementRepository extends BaseRepository<Announcement, Long> {
     ...
     * 根据发布人查询公告
     */
    List<Announcement> findByPublishedByAndDeletedIsFalse(Long publishedBy);
    
    /**
     * 按时间范围查询公告
     */
    List<Announcement> findByPublishedAtBetweenAndDeletedIsFalse(LocalDateTime startDateTime, LocalDateTime endDateTime);
     ...
     */
    @Modifying
    @Query("UPDATE Announcement a SET a.deleted = true, a.deletedAt = CURRENT_TIMESTAMP WHERE a.id IN :ids AND a.deleted = false")
...
public class UserServiceImpl implements UserService {
    ...
    public User login(String username, String password) {
        ...
        if (userOptional.isPresent()) {
            ...
            if (passwordEncoder.matches(password, user.getPassword())) {
                ...
                cacheManager.getCache("loginCache").evict(failKey);
                ...
                Integer failCount = cache.get(failKey, Integer.class);
                if (failCount == null) {
                    failCount = 0;
                }
                failCount++;
    ...
    public List<String> getUserPermissions(Long userId) {
        ...
        List<String> permissions = cache.get(cacheKey, List.class);
...
public class ReviewServiceImpl implements ReviewService {
    ...
    public Review submitApplication(Long userId, Long scholarshipTypeId) throws Exception {
        ...
        if (scholarshipTypeId == null) {
            throw new IllegalArgumentException("奖学金类型ID不能为空");
        }
        ...
        ScholarshipType scholarshipType = scholarshipTypeRepository.findById(scholarshipTypeId)
                .orElseThrow(() -> new Exception("未找到奖学金类型信息"));
        
        // 检查学生是否已经提交过该奖学金的申请（排除rejected状态）
        List<Review> existingApplications = reviewRepository.findByStudentIdAndScholarshipTypeId(student.getId(), scholarshipTypeId);
        boolean hasPendingOrApprovedApplication = existingApplications.stream()
                .anyMatch(app -> !app.getReviewStatus().equals("rejected"));
        
        if (hasPendingOrApprovedApplication) {
            throw new Exception("您已经提交过该奖学金的申请，请勿重复提交");
        }
        
        // 检查学生已提交的申请数量（最多允许2个非rejected状态的申请）
        List<Review> activeApplications = reviewRepository.findByStudentId(student.getId());
        long activeApplicationCount = activeApplications.stream()
                .filter(app -> !app.getReviewStatus().equals("rejected"))
                .count();
        
        if (activeApplicationCount >= 2) {
            throw new Exception("每人最多只能提交2个奖学金申请，已达上限");
        }
        ...
        review.setReviewStatus("pending"); // 默认待审核
        review.setAcademicYear("2024-2025"); // 默认当前学年
        
        // 设置默认评审人（可以根据实际需求调整）
        User defaultReviewer = userRepository.findById(1L)
                .orElseThrow(() -> new Exception("未找到默认评审人"));
        review.setReviewer(defaultReviewer);
        review.setReviewerId(defaultReviewer.getId());
    ...
    public ResponseEntity<?> logout(Authentication authentication, 
        ...
        try {
            ...
            if (token != null && tokenProvider.validateToken(token)) {
                ...
                cacheManager.getCache("tokenCache").put(blacklistKey, "1");
            }
...
#### 7.2.3 StudentScholarshipController - 学生奖学金申请控制器
```java
...
#### 7.2.4 ScholarshipController - 奖学金管理控制器
```java
@RestController
